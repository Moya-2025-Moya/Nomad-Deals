      ...
      <div className="grid grid-cols-[1fr_420px] gap-16 max-w-4xl w-full items-center">
        ...
        <div className="animate-fade-up">
          ...
          <h1 className="mt-8 text-4xl font-bold tracking-tight leading-tight">
            Start finding deals<br />
            <span className="text-gradient">that actually save.</span>
          </h1>
          <p className="mt-4 text-text-secondary leading-relaxed max-w-sm">
            Join travellers who save an average of 42% on flights. Free, forever. No credit card needed.
          </p>
          <div className="mt-8 space-y-3">
            {[
              "See real savings vs. normal fares",
              "AI-powered search for any trip style",
              "Convenience warnings before you book",
            ].map((item) => (
              <div key={item} className="flex items-center gap-3 text-sm text-text-secondary">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-green-dim text-green text-xs border border-green-border">✓</span>
                {item}
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-border bg-bg-card p-8">
            ...
            <div className="space-y-3.5">
              <div>
                ...
                <input type="text" placeholder="Ada Lovelace" className="mt-1.5 w-full rounded-xl border border-border bg-bg-input px-4 py-2.5 text-sm text-text-primary placeholder:text-text-muted outline-none transition focus:border-accent/50" readOnly />
              </div>
              <div>
